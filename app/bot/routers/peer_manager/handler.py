...
async def show_peers_for_server(callback: CallbackQuery, session, server_id=None):
    ...
    if not server or not api_data:
        ...
        await callback.message.edit_text(
            f"<b>{server.name if server else 'Server'}</b>\n<blockquote>Server is not available.</blockquote>",
            reply_markup=servers_list_keyboard(await get_all_servers(), add_back=True),
            parse_mode="HTML"
        )
        return
